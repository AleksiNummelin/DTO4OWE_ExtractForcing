...
hl_variables = {'u':{'stream':'clte','levtype':'hl','freq':'hourly'},
                'v':{'stream':'clte','levtype':'hl','freq':'hourly'},}
...
store['hl_hourly'] = PolytopeZarrStore.from_climate_dt(
            models=[model],
            experiment=exp,
            resolution="high",
            levtype='hl',
...
if download_sfc:
    ...
    # set encoding for saving the data
    encoding={}
    ...
    for month in range(1,13):
        ...
        ds_latlon = ds[fvariables].polytope.sel(model=model,time=timeslice,area=area_NSBS,grid="0.05/0.05")
        outfile   = outpath+model+'/'+exp+'/sfc_'+model+'_'+exp+'_'+str(year)+'_'+str(month).zfill(2)+'.nc'
...
##########################
# DOWNLOAD HEIGHT LEVELS #
##########################
if download_hl:
    hlvariables=list(hl_variables.keys())
    encoding={}
    for var in hlvariables:
        encoding[var]={"zlib": True, "complevel": 5}
    #
    ds = store['hl_hourly'].open()
    for month in range(1,13):
        print(year,month)
        days_in_month = monthrange(year,month)[1]
        timeslice=slice(str(year)+"-"+str(month).zfill(2)+"-01T00:00", str(year)+"-"+str(month).zfill(2)+"-"+str(days_in_month).zfill(2)+"T23:00")
        ds_latlon = ds[hlvariables].polytope.sel(model=model,time=timeslice,area=area_NSBS,grid="0.05/0.05")
        outfile   = outpath+model+'/'+exp+'/hl_'+model+'_'+exp+'_'+str(year)+'_'+str(month).zfill(2)+'.nc'
        for var in hlvariables:
            del ds_latlon[var].attrs['_earthkit']
        #
        ds_latlon.to_netcdf(outfile,format="NETCDF4",unlimited_dims='time',encoding=encoding)
